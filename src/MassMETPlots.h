...
// Tue Apr 10 12:22:46 2018 by ROOT version 5.34/37
// from TTree tree/all observables, signal
// found on file: SkimFileMass.root
//////////////////////////////////////////////////////////

#ifndef MassMETPlots_h
#define MassMETPlots_h
...
public :
   ...
   TBranch        *b_nAK8;   //!
   TString treename_;
   MassMETPlots(TString treename,TTree *tree=0);
   virtual ~MassMETPlots();
...
#ifdef MassMETPlots_cxx
MassMETPlots::MassMETPlots(TString treename,TTree *tree) : fChain(0) 
{
...
// used to generate this class and read the Tree.
  treename_=treename;
   if (tree == 0) {
      ...
      f->GetObject(treename_,tree);

   }
   Init(tree);
}

MassMETPlots::~MassMETPlots()
{
   if (!fChain) return;
   delete fChain->GetCurrentFile();
}

Int_t MassMETPlots::GetEntry(Long64_t entry)
{
// Read contents of entry.
   if (!fChain) return 0;
   return fChain->GetEntry(entry);
}
Long64_t MassMETPlots::LoadTree(Long64_t entry)
{
...
void MassMETPlots::Init(TTree *tree)
{
...
void MassMETPlots::Show(Long64_t entry)
{
...
Int_t MassMETPlots::Cut(Long64_t entry)
{
// This function may be called from Loop.
    if( MET>300 && BTags==0 &&  JetPt1>300  && JetPt2>200 && nAK8>1 && HT>500)return 1;
 ////returns  1 if entry is accepted.   // if( MET>300 && nAK8>1 )return 1;
 else return -1;
// returns -1 otherwise.
   //return 1;
}
#endif // #ifdef MassMETPlots_cxx
